...
var replClassicCmd = &cobra.Command{
	Use:   "repl-classic",
	Short: "Start the classic Readline REPL session",
	Long:  "Start the classic scrolling prompt for searching words, kanji, and names.",
	Args:  cobra.NoArgs,
	RunE: func(cmd *cobra.Command, args []string) error {
		return runClassicREPLWithQuerier(cmd.Context(), query.New(db))
	},
}

func init() {
	rootCmd.AddCommand(replClassicCmd)
}
...
func runClassicREPLWithQuerier(ctx context.Context, q query.Querier) error {
...
func replDispatch(ctx context.Context, rl *readline.Instance, q query.Querier, line string) error {
	if strings.HasPrefix(line, "/") {
		...
		case "th":
			if len(args) == 0 {
				fmt.Fprintln(os.Stderr, "usage: /th <English word or phrase>")
				return nil
			}
			return lookupThaiTo(ctx, os.Stdout, strings.Join(args, " "))
		case "search":
...
func replPrintHelp() {
  ...
  /th <English word or phrase>                 look up Thai meanings offline
